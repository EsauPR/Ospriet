...
.methods({
	...
	initTimeline: function () {
		var params = {
			user_id: this.account_details.id_str,
			count: 200
		};

		this.twttr.get('statuses/user_timeline', params, function(error, tweets, response) {
			if (error) {
				console.log(error)
			} else {
				//console.log('Timeline: ', data)
				this.account_statuses = tweets;		
			}
		}.bind(this));
		/*
		this.twttr.getUserTimeline({user_id:this.account_details.id_str, count:200, include_entities: true}, function(err, data) {
			if (err) {
				console.log(err)
				console.log('Error getting initTimeline');
			} else {
				//console.log('Timeline: ', data)
	...
	filter: function (data) {
		...
		if(!blocked) {
			...
			if (data.in_reply_to_user_id_str &&
				...
				this.twttr.post('statuses/update', params, function (err, tweet, response){
					if(err) {
						return console.log('error on posting: '+err);
					}
					// prepend tweet to array to maintain reverse chronological order
					this.account_statuses.splice(0,0,tweet);
